	...
	for (int l = 0; l < tilemap->num_layers; ++l) {
		...
		for (int y = 0; y < mapheight; ++y) {
			for (int x = 0; x < mapwidth; ++x) {
				...
				const char tile_char = layer->offset_to_char[offset];
				...
				const float walking_speed = tile_properties->enum_to_walking_speed[tile_enum];
				const float current_walking_speed = tilemap->collision_buffer.offset_to_walking_speed[offset];
	...
	const f32seconds dt)
{
	...
	const int offset = tile_y * mapwidth + tile_x;
